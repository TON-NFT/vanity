	...
	for {
		...
		for i := uint32(0); i < 1_000_000_000; i++ {
			...
			if equalityFunc(suffix, string(addrTo[strCmpOffset:])) {
				...
				nsec := now.Unix()
				d1 := []byte(addr.String())
				d2 := []byte(hex.EncodeToString(pk.Seed()))
				filename1 := fmt.Sprint("./wallets/wallet_address_", nsec)
				filename2 := fmt.Sprint("./wallets/private_key_", nsec)
				err := os.WriteFile(filename1, d1, 0644)
				check(err)
				err2 := os.WriteFile(filename2, d2, 0644)
				check(err2)
				log.Println(
					"========== FOUND ==========\n",
